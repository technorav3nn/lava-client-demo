...
@CommandBuilder(
    { name: "leave", description: "Leaves the VC that the bot is currently in." },
    {
        group: "music",
    }
)
export default class Leave extends Command {
    async exec(ctx: CommandContext) {
        ...
        if (!player?.connected) {
            return ctx.reply(Utils.embed("I couldn't find a player for this guild."), {
                ephemeral: true,
            });
        }
        ...
        if (!vc || player.channelId !== vc.id) {
            return ctx.reply(Utils.embed("You're not in my voice channel, bozo."), {
                ephemeral: true,
            });
        }
